...
public class RouteSingleTile extends Tile {
    ...
    public RouteSingleTile(RouteScene parent, int cityID, String date){
        super(parent);
        this.date = date;
        try{
            weather = WeatherAPI.makeRequest(RequestType.Current, cityID)[0];
        } catch(IOException e) {
            System.out.println("Location ID (" + cityID + ") not found in weather service.");
            weather = new Weather(0.1, 0.1, "Cloudy", "chance of meatballs", -3.14, 101325, 95.25, -3.142, -3.14, 1E-6, 66.6, 2 * Math.PI, 42.0, "Cambridge", 2653941, "2012 AD", 123.321);
        }
        ...
        //Add city label
        cityLabel = new AutoSizeText(weather.getCity_name(), Settings.getPrimary());
    ...
    public void update(){
        ...
        cityLabel.setText(weather.getCity_name());
